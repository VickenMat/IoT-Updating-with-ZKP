  ...
  describe("Minting", function () {
    ...
    it("should fail to mint more NFTs than the max supply", async () => {
      ...
      for (let i = 0; i < maxSupply; i++) {
        await MintUpdateNFTContractLimitedInstance.safeMint(owner.address, "mockZKPProof");
      }

      // Attempting to mint beyond the limit should fail
      await expect(MintUpdateNFTContractLimitedInstance.safeMint(owner.address, "mockZKPProof")).to.be.revertedWith(
